...
* *Scenario 1: Deploy and Configure {partner-product-name} and your own Active Directory Domain Services (AD DS) installation on Amazon EC2 instances.* The AWS CloudFormation template for this scenario builds the AWS Cloud infrastructure, sets up and configures AD DS and AD-integrated Domain Name Service (DNS) on the AWS Cloud. It also creates AWS Directory Service (AD Connector) but you handle maintenance and adminstration of AD DS tasks. It then deploys Amazon WorkSpaces and FreeRADIUS solution that is integrated with AD DS, configured and enabled on the AD Connector. You can also choose to deploy into your existing VPC infrastructure. Manual steps are required for users to register MFA token after deployment. 
